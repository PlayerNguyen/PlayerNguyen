...
export default function GraphMaze(props: MazeProps) {
  ...
  const [isVisualizing, setVisualizing] = useState<boolean>(false);
    ...
    let intervalId = setInterval(() => {
      ...
      for (let i = 0; i < vDirection.length; i++) {
        ...
        if (nextPoint.x === destination.x && nextPoint.y === destination.y) {
          ...
          let cur = detail[destination.y][destination.x];
          // console.log(cur, cur.parent);
          let curLocation = { x: destination.x, y: destination.y };
          while (cur.parent.x !== -1 && cur.parent.y !== -1) {
            // console.log(cur);
            const nodeType = getNodeType(curLocation.x, curLocation.y);
            if (
              nodeType !== NodeType.SOURCE &&
              nodeType !== NodeType.DESTINATION
            ) {
              setNodeType(curLocation.x, curLocation.y, NodeType.PATH);
            }
            curLocation = cur.parent;
            cur = detail[cur.parent.y][cur.parent.x];
          }
          return;
        }
  ...
  return (
    <div className={classNames(`flex flex-col items-center py-4`)}>
              ...
              return (
                <div
                  ...
                  className={classNames(
                    `inline-block hover:bg-neutral-600 relative first:border-l last:border-r border-neutral-700`,
                    ` h-2 w-2 md:h-3 md:w-3 lg:w-4 lg:h-4`,
                    {
                  ...
                  onDoubleClick={() => {
                    if (!isVisualizing) updateSource(_xIndex, _yIndex);
                  }}
      ...
      {/* Description */}
      <div className={classNames(`flex flex-row lg:flex-col`, `text-sm my-4`)}>
      ...
      <div className="flex flex-row gap-4 items-center text-neutral-500">
        {/* Visualize options */}
        <div
          className={classNames(`flex flex-col lg:flex-row gap-4 items-center`)}
        >
        ...
        <div className={classNames(`flex flex-col gap-2 lg:flex-row`)}>
          <Button
            onClick={() => resetGraph()}
          ...
          <Button
            onClick={() => {
              visualAbortController.abort(`Canceled by user`);
            }}
            // disabled={!isVisualizing}
            text="Cancel"
          />
